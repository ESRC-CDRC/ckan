...
def markdown_extract(text, extract_length=190):
    ...
    if (text is None) or (text.strip() == ''):
        return ''
    plain = RE_MD_HTML_TAGS.sub('', markdown2.markdown(text, extras=['tables', 'fenced-code-blocks']))
...
def render_markdown(data, auto_link=True, allow_html=False):
    ...
    if not data:
        return ''
    if allow_html:
        data = markdown2.markdown(data.strip(), extras=['tables', 'fenced-code-blocks'])
    else:
        data = RE_MD_HTML_TAGS.sub('', data.strip())
        data = markdown2.markdown(data, extras=['tables', 'fenced-code-blocks'])
